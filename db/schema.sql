...
-- -----------------------------------------------------
-- Table `pine`.`indicator_has_lookup_data`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pine`.`indicator_has_lookup_data` (
  ...
  CONSTRAINT `fk_indicator_has_lookup_data_lookup_data_id`
    ...
    REFERENCES `pine`.`lookup_data` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `pine`.`lookup_data`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pine`.`lookup_data` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `update_timestamp` TIMESTAMP NOT NULL,
  `create_timestamp` TIMESTAMP NOT NULL,
  `lookup_id` INT UNSIGNED NOT NULL,
...
-- -----------------------------------------------------
-- Table `pine`.`qnaire_has_lookup`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pine`.`qnaire_has_lookup` (
  `qnaire_id` INT(10) UNSIGNED NOT NULL,
  `lookup_id` INT UNSIGNED NOT NULL,
  `update_timestamp` TIMESTAMP NOT NULL,
  `create_timestamp` TIMESTAMP NOT NULL,
...
CREATE TABLE IF NOT EXISTS `pine`.`question` (
  ...
  `rank` INT(10) UNSIGNED NOT NULL,
  `name` VARCHAR(255) NOT NULL,
  `type` ENUM('audio', 'boolean', 'comment', 'date', 'device', 'list', 'lookup', 'lookup-indicator', 'number', 'string', 'text') NOT NULL,
...
FOR EACH ROW
BEGIN   CALL update_respondent_current_response( OLD.respondent_id );    SELECT COUNT(*) INTO @test FROM response WHERE respondent_id = OLD.respondent_id;   IF 0 = @test THEN     UPDATE respondent SET end_datetime = NULL WHERE id = OLD.respondent_id;   END IF; END$$
