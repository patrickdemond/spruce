...
-- Fri 27 Mar 2020 04:23:12 PM EDT
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`response_BEFORE_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`response_BEFORE_INSERT` BEFORE INSERT ON `response` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`response_BEFORE_UPDATE` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`response_BEFORE_UPDATE` BEFORE UPDATE ON `response` FOR EACH ROW
BEGIN
