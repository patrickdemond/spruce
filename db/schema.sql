...
-- Wed 27 Nov 2019 03:39:10 PM EST
...
-- -----------------------------------------------------
-- Table `pine`.`answer_has_question_option`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`answer_has_question_option` ;

CREATE TABLE IF NOT EXISTS `pine`.`answer_has_question_option` (
...
-- -----------------------------------------------------
-- Table `pine`.`answer_extra`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`answer_extra` ;

CREATE TABLE IF NOT EXISTS `pine`.`answer_extra` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `update_timestamp` TIMESTAMP NOT NULL,
  `create_timestamp` TIMESTAMP NOT NULL,
  `answer_id` INT UNSIGNED NOT NULL,
...
USE `pine`;
DROP procedure IF EXISTS `pine`.`update_reqn_current_reqn_version`;
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`answer_BEFORE_UPDATE` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`answer_BEFORE_UPDATE` BEFORE UPDATE ON `answer` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`answer_has_question_option_AFTER_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`answer_has_question_option_AFTER_INSERT` AFTER INSERT ON `answer_has_question_option` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`answer_extra_AFTER_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`answer_extra_AFTER_INSERT` AFTER INSERT ON `answer_extra` FOR EACH ROW
BEGIN
  ...
  AND ( SELECT exclusive FROM question_option WHERE id = answer_has_question_option.question_option_id );
END$$


USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`answer_extra_BEFORE_UPDATE` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`answer_extra_BEFORE_UPDATE` BEFORE UPDATE ON `answer_extra` FOR EACH ROW
BEGIN
  -- make sure that only one value can be non-null
  IF( OLD.value_boolean IS NULL AND NEW.value_boolean IS NOT NULL ) THEN
    SET NEW.value_number = NULL, NEW.value_string = NULL, NEW.value_text = NULL;
