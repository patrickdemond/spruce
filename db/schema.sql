...
-- Schema pine
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `pine` DEFAULT CHARACTER SET utf8mb4 ;
USE `cenozo` ;
...
CREATE TABLE IF NOT EXISTS `pine`.`answer` (
  ...
  `language_id` INT(10) UNSIGNED NOT NULL,
  `user_id` INT(10) UNSIGNED NULL DEFAULT NULL,
  `value` LONGTEXT CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_bin' NOT NULL DEFAULT 'null',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `pine`.`qnaire` (
  ...
  `beartooth_url` VARCHAR(255) NULL DEFAULT NULL,
  `beartooth_username` VARCHAR(255) NULL DEFAULT NULL,
  `beartooth_password` VARCHAR(255) NULL DEFAULT NULL,
  `description` TEXT NULL DEFAULT NULL,
  `note` TEXT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uq_name` (`name` ASC),
...
CREATE TABLE IF NOT EXISTS `pine`.`question` (
  ...
  `rank` INT(10) UNSIGNED NOT NULL,
  `name` VARCHAR(255) NOT NULL,
  `type` ENUM('boolean', 'comment', 'date', 'device', 'list', 'number', 'string', 'text') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `pine`.`response` (
  ...
  `current_page_rank` INT UNSIGNED NULL DEFAULT NULL,
...
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`alternate_AFTER_INSERT` AFTER INSERT ON `cenozo`.`alternate` FOR EACH ROW
BEGIN
...
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`alternate_consent_type_AFTER_INSERT` AFTER INSERT ON `alternate_consent_type` FOR EACH ROW
BEGIN
...
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`page_AFTER_DELETE` AFTER DELETE ON `page` FOR EACH ROW
BEGIN
  -- update the qnaire's total number of pages
...
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`respondent_AFTER_INSERT` AFTER INSERT ON `respondent` FOR EACH ROW
BEGIN
...
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`stage_BEFORE_INSERT` BEFORE INSERT ON `stage` FOR EACH ROW
BEGIN
