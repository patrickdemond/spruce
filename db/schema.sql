...
-- Table `pine`.`service`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`service` ;
...
-- Table `pine`.`writelog`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`writelog` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`role_has_service` ;
...
-- Table `pine`.`setting`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`setting` ;
...
-- Table `pine`.`access`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`access` ;
...
-- Table `pine`.`qnaire`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`qnaire` ;
...
-- Table `pine`.`module`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`module` ;
...
-- Table `pine`.`page`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`page` ;
...
-- Table `pine`.`question`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`question` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`attribute` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`respondent` ;
...
-- Table `pine`.`response`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`response` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`response_attribute` ;
...
-- Table `pine`.`answer`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`answer` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`question_option` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`qnaire_has_language` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`module_description` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`page_description` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`question_description` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`question_option_description` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`qnaire_description` ;
...
CREATE TABLE IF NOT EXISTS `pine`.`qnaire_description` (
  ...
  `qnaire_id` INT UNSIGNED NOT NULL,
  `language_id` INT UNSIGNED NOT NULL,
  `type` ENUM('introduction', 'conclusion', 'closed', 'invitation subject', 'invitation body') NOT NULL,
  `value` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_qnaire_id` (`qnaire_id` ASC),
  INDEX `fk_language_id` (`language_id` ASC),
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`page_time` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`respondent_current_response` ;
...
-- Table `pine`.`reminder`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`reminder` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`respondent_mail` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`qnaire_consent_type` ;
...
CREATE TABLE IF NOT EXISTS `pine`.`qnaire_consent_type` (
  ...
  INDEX `fk_consent_type_id` (`consent_type_id` ASC),
  INDEX `fk_question_id` (`question_id` ASC),
  UNIQUE INDEX `uq_qnaire_id_consent_type_id_question_id_accept` (`qnaire_id` ASC, `consent_type_id` ASC, `question_id` ASC, `accept` ASC),
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`qnaire_average_time` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`module_average_time` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`page_average_time` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pine`.`reminder_description` ;
...
USE `pine`;
DROP procedure IF EXISTS `pine`.`update_respondent_current_response`;
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`qnaire_AFTER_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`qnaire_AFTER_INSERT` AFTER INSERT ON `qnaire` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`qnaire_AFTER_UPDATE` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`qnaire_AFTER_UPDATE` AFTER UPDATE ON `qnaire` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`module_AFTER_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`module_AFTER_INSERT` AFTER INSERT ON `module` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`page_AFTER_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`page_AFTER_INSERT` AFTER INSERT ON `page` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`question_BEFORE_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`question_BEFORE_INSERT` BEFORE INSERT ON `question` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`question_AFTER_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`question_AFTER_INSERT` AFTER INSERT ON `question` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`question_BEFORE_UPDATE` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`question_BEFORE_UPDATE` BEFORE UPDATE ON `question` FOR EACH ROW
BEGIN
  -- make sure name is valid
  SELECT NEW.name RLIKE "^[a-z0-9_]+$" INTO @test;
  IF( @test = 0 ) THEN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`attribute_BEFORE_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`attribute_BEFORE_INSERT` BEFORE INSERT ON `attribute` FOR EACH ROW
BEGIN
  -- make sure name is valid
  SELECT NEW.name RLIKE "^[a-z0-9_]+$" INTO @test;
  IF( @test = 0 ) THEN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`attribute_BEFORE_UPDATE` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`attribute_BEFORE_UPDATE` BEFORE UPDATE ON `attribute` FOR EACH ROW
BEGIN
  -- make sure name is valid
  SELECT NEW.name RLIKE "^[a-z0-9_]+$" INTO @test;
  IF( @test = 0 ) THEN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`response_AFTER_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`response_AFTER_INSERT` AFTER INSERT ON `response` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`response_AFTER_DELETE` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`response_AFTER_DELETE` AFTER DELETE ON `response` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`question_option_BEFORE_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`question_option_BEFORE_INSERT` BEFORE INSERT ON `question_option` FOR EACH ROW
BEGIN
  -- make sure name is valid
  SELECT NEW.name RLIKE "^[a-z0-9_]+$" INTO @test;
  IF( @test = 0 ) THEN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`question_option_AFTER_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`question_option_AFTER_INSERT` AFTER INSERT ON `question_option` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`question_option_BEFORE_UPDATE` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`question_option_BEFORE_UPDATE` BEFORE UPDATE ON `question_option` FOR EACH ROW
BEGIN
  -- make sure name is valid
  SELECT NEW.name RLIKE "^[a-z0-9_]+$" INTO @test;
  IF( @test = 0 ) THEN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`qnaire_has_language_AFTER_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`qnaire_has_language_AFTER_INSERT` AFTER INSERT ON `qnaire_has_language` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`qnaire_has_language_AFTER_DELETE` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`qnaire_has_language_AFTER_DELETE` AFTER DELETE ON `qnaire_has_language` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`reminder_AFTER_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`reminder_AFTER_INSERT` AFTER INSERT ON `reminder` FOR EACH ROW
BEGIN
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`respondent_mail_BEFORE_INSERT` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`respondent_mail_BEFORE_INSERT` BEFORE INSERT ON `respondent_mail` FOR EACH ROW
BEGIN
  SET @test = (
...
USE `pine`$$
DROP TRIGGER IF EXISTS `pine`.`respondent_mail_BEFORE_UPDATE` $$
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`respondent_mail_BEFORE_UPDATE` BEFORE UPDATE ON `respondent_mail` FOR EACH ROW
BEGIN
  SET @test = (
    SELECT COUNT(*) FROM respondent_mail
    WHERE respondent_id <=> NEW.respondent_id
