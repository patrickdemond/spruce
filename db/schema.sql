...
-- Table `pine`.`image`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pine`.`image` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `update_timestamp` TIMESTAMP NOT NULL,
  `create_timestamp` TIMESTAMP NOT NULL,
  `qnaire_id` INT(10) UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `pine`.`question` (
  ...
  `rank` INT(10) UNSIGNED NOT NULL,
  `name` VARCHAR(255) NOT NULL,
  `type` ENUM('audio', 'boolean', 'comment', 'date', 'device', 'list', 'number', 'string', 'text') NOT NULL,
...
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`response_AFTER_INSERT` AFTER INSERT ON `pine`.`response` FOR EACH ROW
BEGIN
...
USE `pine`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pine`.`response_AFTER_DELETE` AFTER DELETE ON `pine`.`response` FOR EACH ROW
BEGIN
