...
INSERT IGNORE INTO patrick_linden.qnaire( name ) VALUES ( "Tracking F2 Main" );
...
FROM patrick_linden.qnaire, patrick_limesurvey.groups
...
JOIN patrick_linden.module ON group_order+1 = module.rank
JOIN patrick_linden.qnaire ON module.qnaire_id = qnaire.id
WHERE qnaire.name = "Tracking F2 Main"
...
ALTER TABLE patrick_linden.question
ADD COLUMN qid INT NULL DEFAULT NULL,
ADD INDEX dk_qid ( qid );

-- add all questions to the pages
INSERT IGNORE INTO patrick_linden.question( qid, page_id, rank, name, description, type, multiple )
...
UPDATE patrick_linden.question_option
JOIN patrick_linden.question ON question_option.question_id = question.id
