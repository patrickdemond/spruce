...
BEGIN
  ...
  WHERE qnaire.id = ( SELECT qnaire_id FROM module WHERE id = NEW.module_id );

  UPDATE qnaire SET total_pages = @pages;
...
BEGIN
  ...
  WHERE qnaire.id = ( SELECT qnaire_id FROM module WHERE id = OLD.module_id );
