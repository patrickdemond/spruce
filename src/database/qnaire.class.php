...
class qnaire extends \cenozo\database\record
{
  ...
  public function export_respondent_data( $db_specific_respondent = NULL )
  {
    ...
    foreach( $file_data as $index => $file )
    {
      ...
      {
        $this->send_data_to_parent_pine( 'files', $partial_file_data );
  ...
  public function import_device_files( $file_list )
  {
    ...
    {
      $db_question = $this->get_question( $file->question );
      ...
      $directory = sprintf( '%s/%s', $db_question->get_data_directory(), $file->uid );
  ...
  public function get_response_metadata( $modifier = NULL )
  {
    ...
    if( $this->stages ) $header = array_merge( $header, ['interview_type'] );
    $header = array_merge( $header, $column_list );
  ...
  /**
   * An internal function for exporting data to parent pine instance
   * @param string @type What type of data to export ("responses" or "files")
   * @param array $data The data to send
   */
  private function send_data_to_parent_pine( $type, $data )
  {
    $url = sprintf(
      '%s/api/qnaire/name=%s/respondent?action=import_%s',
      PARENT_INSTANCE_URL,
      util::full_urlencode( $this->name ),
      $type
    );
