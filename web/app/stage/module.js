cenozoApp.defineModule( { name: 'stage', models: ['add', 'list', 'view'], create: module => {
  ...
  cenozo.providers.directive( 'cnStageAdd', [
    'CnStageModelFactory', 'CnHttpFactory',
        ...
        controller: function( $scope ) {
          ...
          $scope.$on( 'cnRecordAdd linked', function( event, data ) { 
            ...
            cnRecordAddScope.check = async function( property ) { 
              ...
              if( ['rank', 'first_module_id', 'last_module_id'].includes( property ) ) {
                var inputArray = cnRecordAddScope.dataArray.findByProperty( 'title', '' ).inputArray;
                $scope.model.updateModuleListState(
                  inputArray.findByProperty( 'key', 'first_module_id' ).enumList,
                  inputArray.findByProperty( 'key', 'last_module_id' ).enumList,
                  cnRecordAddScope.record.rank ?
                    $scope.model.stageList.findByProperty( 'rank', cnRecordAddScope.record.rank - 1 ) : null,
                  cnRecordAddScope.record.rank ?
                    $scope.model.stageList.findByProperty( 'rank', cnRecordAddScope.record.rank ) : null,
        ...
        angular.extend( this, {
          onView: async function( force ) {
            ...
            this.parentModel.updateModuleListState(
              ...
              this.parentModel.stageList.findByProperty( 'rank', this.record.rank - 1 ),
              this.parentModel.stageList.findByProperty( 'rank', this.record.rank + 1 ),
      ...
      var object = function( root ) {
        ...
        angular.extend( this, {
            ...
            var response = await CnHttpFactory.instance( {
              ...
              data: {
                select: {
                  column: [
                    'rank',
                    { table: 'first_module', column: 'rank', alias: 'first_module_rank' },
                    { table: 'last_module', column: 'rank', alias: 'last_module_rank' },
                  ]
                },
                modifier: {
                  join: [
                    { table: 'module', onleft: 'stage.first_module_id', onright: 'first_module.id', alias: 'first_module' },
                    { table: 'module', onleft: 'stage.last_module_id', onright: 'last_module.id', alias: 'last_module' }
                  ],
                  order: 'stage.rank'
                }
              }
          ...
          updateModuleListState: function( firstEnumList, lastEnumList, prevStage, nextStage, firstModuleId, lastModuleId ) {
            ...
            var minModuleRank = angular.isDefined( firstEnumList[0].rank ) ? firstEnumList[0].rank : firstEnumList[1].rank;
            ...
            // one then it has to retain at least one module
            var minRank = !nextStage ? maxModuleRank : prevStage ? prevStage.first_module_rank : 1;
