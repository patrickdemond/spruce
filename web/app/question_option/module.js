      ...
      return {
        ...
        controller: async function( $scope ) {
          ...
          CnSession.setBreadcrumbTrail( [ {
            title: 'Page',
            go: async function() { await $state.go( 'question.list' ); }
          }, {
            title: $scope.model.parentSourceName,
            go: async function() { await $state.go( 'question.view', { identifier: $scope.model.sourceParentId } ); }
          }, {
            title: 'Question Options'
          }, {
            title: $scope.model.sourceName,
            go: async function() { await $state.go( 'question_option.view', { identifier: $scope.model.sourceId } ); }
          }, {
            title: 'move/copy'
          } ] );
        }
